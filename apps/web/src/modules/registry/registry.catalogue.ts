...
/*
 * The item plus every registry dependency it reaches, pro items excluded -
 * a free component must not smuggle a paywalled one's source into its
 * payload just by depending on it.
 */
function closureOf(item: RegistryItem): readonly RegistryItem[] {
	const seen = new Map<string, RegistryItem>();

	const walk = (part: RegistryItem): void => {
		if (seen.has(part.name)) return;
		seen.set(part.name, part);

		for (const name of part.registryDependencies ?? []) {
			const dep = findRegistryItem(name);
			if (dep && dep.access !== "pro") walk(dep);
		}
	};
...
export function toTanStackAddOn(
	...
	/*
	 * One self-contained document, dependencies included.
	 *
	 * The released TanStack CLI resolves `dependsOn` only against its own
	 * built-in add-ons - a bare id from a third-party registry fails the
	 * whole install, even when that dependency's URL is in the same batch.
	 * So dependencies are not declared here, they are carried: the closure's
	 * files and npm packages are inlined, and the stylesheet rides along
	 * exactly as it does in the shadcn shape. Two add-ons that share a
	 * dependency write it twice with identical bytes, which is a no-op.
	 */
	const files: Record<string, string> = {};
	const packages: Record<string, string> = {};

	for (const part of closureOf(item)) {
		Object.assign(files, filesFor(part));
		Object.assign(packages, part.dependencies);
	}

	files["src/sushindustries/atoms.css"] = bundledAtomsCss();
	...
	return {
		...
		packageAdditions:
			Object.keys(packages).length > 0 ? { dependencies: packages } : {},
		files,
		deletedFiles: [],
	};
}
