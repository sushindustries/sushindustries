...
const OPTIONS = {
	 ...
	 * browser's own noise, not this site's - Chrome and Safari throw it
	 * whenever an observed element's own resize handler resizes it again in
	 * the same frame (the product viewer's canvas, most likely), it self
	 * -recovers on the next frame, and nothing a visitor can see is broken.
	 * Dropped here rather than filtered on the dashboard, so it never
	 * consumes an event quota or an error-tracking issue in the first place.
	 */
	before_send: ((event) => {
		if (event?.event !== "$exception") return event;

		const isResizeObserverNoise = (
			event.properties?.$exception_list as Array<{ value?: string }> | undefined
		)?.some((exception) =>
			exception.value?.includes("ResizeObserver loop completed"),
		);

		return isResizeObserverNoise ? null : event;
